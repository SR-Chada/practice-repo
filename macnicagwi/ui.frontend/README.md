...
### Aemfed (Speed up your AEM front-end development using aemfed.)

1. Run `npm install aemfed --global` to install aemfed globally
2. Open PowerShell as an Administrator and run the command `Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy Unrestricted`. This sets current windows user to run ps1 files.
3. Open terminal in your VSCode or command prompt within your project and run the command `aemfed -t "http://admin:admin@localhost:4502" -w "ui.apps/src/main/content/jcr_root/"` to start aemfed.
4. Navigate to the page you want to work on and make some changes to the clientlib files under the path provided during step 3. When saving the file, aemfed reports the detected file changes and uploads them to AEM.
5. Refresh the page you are working on AEM instance and check your changes. 
6. If you are working in ui.frontend, then run `npm run prod` inside ui.frontend path in the terminal. If aemfed is already running, it will detect the changes and uploads to AEM. Else repeat step 3 after running `npm run prod` command. 
7. Github link: https://github.com/abmaonline/aemfed