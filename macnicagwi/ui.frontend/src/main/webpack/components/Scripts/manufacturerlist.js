...
class ManufacturerList extends MultilevelFilter {
    constructor(componentContainer, dataResourcePath) {
        super(componentContainer, dataResourcePath);
        this.itemsTag = ".cmp-manufacturerlist__entry";
    }
    ...
    updateComponentDom(apiResponse, isLoadMore) {
        ...
        apiResponse.done((data)=>{
            const cardlistContainer = $('<div />').append(data).find('.cmp-manufacturerlist__entries').html();
            const totalResultsSize = $('<div />').append(data).find('.cmp-manufacturerlist').attr("data-cmp-listsize");
            const isDataAvailable = $('<div />').append(data).find('.cmp-manufacturerlist li').length != 0 ? true: false;
            if(isDataAvailable) {
                this.componentContainer.find(".cmp-manufacturerlist__no-results-found").hide();
                if(isLoadMore) {
                    $("body").find(this.componentContainer).find(".cmp-manufacturerlist__entries").append(cardlistContainer);
                } else {
                    $("body").find(this.componentContainer).find(".cmp-manufacturerlist__entries").html(cardlistContainer);
                }
            } else {
                $("body").find(this.componentContainer).find(".cmp-manufacturerlist").html("");
                this.componentContainer.find(".cmp-manufacturerlist__loadmore-button > .cmp-button").attr("disabled",true);
                this.componentContainer.find(".cmp-manufacturerlist__no-results-found").show();
            }
           
            $("body").find(this.componentContainer).find(".cmp-manufacturerlist").attr("data-cmp-listsize", totalResultsSize);
    ...
    loadMoreFunctionality() {
        ...
        this.componentContainer.find(".cmp-manufacturerlist__loadmore-button > .cmp-button").on("click",function() {
            ...
            const totalResultsSize = self.componentContainer.find(".cmp-manufacturerlist").attr("data-cmp-listsize");
...
$(document).ready(function() {
    $(".manufacturerlist").trigger("load");
});

$(".manufacturerlist").on("load",function() {
    const componentContainer = $(this);
    const dataResourcePath = $(this).find(".cmp-manufacturerlist").attr("data-cmp-resource");
    const manufacturerListClass = new ManufacturerList(componentContainer, dataResourcePath);
    manufacturerListClass.init();
