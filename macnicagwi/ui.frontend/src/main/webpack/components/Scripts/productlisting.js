...
class ProductListing extends MultilevelFilter {
    ...
    showNoResultsFound(isDataAvailable) {
        if(isDataAvailable) {
            this.componentContainer.find(".cmp-productlisting__no-results-found").hide();
        } else {
            this.componentContainer.find(".cmp-productlisting__loadmore-button > .cmp-button").attr("disabled",true);
            this.componentContainer.find(".cmp-productlisting__no-results-found").show();
        }
    }
    ...
    updateComponentDom(apiResponse, isLoadMore) {
        ...
        apiResponse.done((data)=>{
            const cardlistContainer = $('<div />').append(data).find('.cmp-list').html();
            const totalResultsSize = $('<div />').append(data).find('.cmp-productlisting').attr("data-cmp-listsize");
            const isDataAvailable = $('<div />').append(data).find('.cmp-productlisting li').length != 0 ? true: false;
            if(isDataAvailable) {
                this.componentContainer.find(".cmp-productlisting__no-results-found").hide();
                if(isLoadMore) {
                    $("body").find(this.componentContainer).find(".cmp-list").append(cardlistContainer);
                } else {
                    $("body").find(this.componentContainer).find(".cmp-list").html(cardlistContainer);
                }
            } else {
                $("body").find(this.componentContainer).find(".cmp-productlisting").html("");
                this.componentContainer.find(".cmp-productlisting__loadmore-button > .cmp-button").attr("disabled",true);
                this.componentContainer.find(".cmp-productlisting__no-results-found").show();
            }
           
            $("body").find(this.componentContainer).find(".cmp-productlisting").attr("data-cmp-listsize", totalResultsSize);
    ...
    uncheckFiltersFunctionality() {
        ...
        this.componentContainer.find(".cmp-multilevelfilter__clear-button").each(function(){
            $(this).on('click',function() {
                ...
                self.componentContainer.find(".cmp-productlisting__loadmore-button > .cmp-button").attr("disabled",false);
            });
        });
    }
    ...
    loadMoreFunctionality() {
        ...
        this.componentContainer.find(".cmp-productlisting__loadmore-button > .cmp-button").on("click",function() {
            const currentListSize = self.componentContainer.find(self.itemsTag).length;
            const totalResultsSize = self.componentContainer.find(".cmp-productlisting").attr("data-cmp-listsize");
...
$(".productlisting").on("load",function() {
    const componentContainer = $(this);
    const dataResourcePath = $(this).find(".cmp-productlisting").attr("data-cmp-resource");
    const productListingClass = new ProductListing(componentContainer, dataResourcePath);
    productListingClass.init();
    $(window).trigger("resize");
});

$(document).ready(function() {
    $(".productlisting").trigger("load");
});